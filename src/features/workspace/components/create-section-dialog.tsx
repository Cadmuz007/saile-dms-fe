...
interface CreateSectionDialogProps {
  ...
  onCreate: (section: { name: string; section: LibrarySection }) => Promise<void>;
}

export function CreateSectionDialog({ defaultSection = "Home", parentName, open, onOpenChange, onCreate }: CreateSectionDialogProps) {
  ...
  useEffect(() => { if (open) reset({ name: "", section: defaultSection }); }, [defaultSection, open, reset]);
  ...
  async function submit(values: SectionFormValues): Promise<void> {
    try { await onCreate({ name: values.name, section: values.section }); closeDialog(); }
    catch (error) { setRequestError(error instanceof Error ? error.message : "The folder could not be created."); }
  }
