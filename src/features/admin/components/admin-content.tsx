...
import { AuthenticationPage } from "./access-management";
...
interface AdminContentProps { activeView: AdminView; currentUser: AuthenticatedUser; onSignOut: () => void; onLicenseMetricsChange: (metrics: LicenseMetrics) => void; }
...
export function AdminContent({ activeView, currentUser, onSignOut, onLicenseMetricsChange }: AdminContentProps) {
  const mainSx = { p: { xs: 2, md: 3.5 } };
  if (activeView === "overview") return <Box component="main" sx={mainSx}><Overview /></Box>;
  if (activeView === "licenses") return <Box component="main" sx={mainSx}><LicensingPage currentUser={currentUser} onSignOut={onSignOut} onMetricsChange={onLicenseMetricsChange} /></Box>;
  if (activeView === "authentication") return <Box component="main" sx={mainSx}><AuthenticationPage currentUser={currentUser} onSignOut={onSignOut} /></Box>;
